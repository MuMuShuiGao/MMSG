...
```
SessionRouter._consume_loop()
│  item = await message_bus.consume_inbound()           ← 从队列拉取 (asyncio.Queue)
│  result = await _process(item.source, text)
...
└─ _process(source, text)
   ...
   └─ agent.run(text)
      │
      ├─ 1. memory.start_turn()                         ← 开启新一轮上下文写入
      │
      ├─ 2. memory.write(user_record)                   ← 用户消息持久化到 current_context.md
      │
      └─ 3. 思考循环 (max_steps=8)
         │
         ├─ _assemble_messages()
...
```
channel.publish_inbound()  →  [_inbound queue]  →  SessionRouter.consume_inbound()  →  AgentLoop.run()
                                                                                            │
channel.callback(item)    ←  [_outbound queue]  ←  SessionRouter.publish_outbound()  ←  return
```

## 关键点

- **consume_inbound**: `asyncio.Queue.get()` 阻塞等待，串行处理每条消息，天然排队。
